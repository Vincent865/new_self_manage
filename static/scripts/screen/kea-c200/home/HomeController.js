function HomeController(viewHandle, elementId) {
    ...
    this.pRun = "#" + this.elementId + '_runLight';
    this.pPower = "#" + this.elementId + '_powerLight';
    this.pFind = "#" + this.elementId + '_findLight';
    this.pAlarm = "#" + this.elementId + '_alarmLight';
    ...
    try {
        ...
        promise.done(function (result) {
            if (typeof (result) != "undefined") {
                ...
                //Power
                if (result.sys_info.powerState == 1) {
                    $(parent.pPower).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].RED);
                }
                else if (result.sys_info.powerState == 2) {
                    $(parent.pPower).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GREEN);
                }
                else {
                    $(parent.pPower).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GRAY);
                }
                //Run
                if (result.sys_info.runState == 4) {
                    $(parent.pRun).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GREEN_BLINK);
                } else {
                    $(parent.pRun).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GRAY);
                }
                //Find
                if (result.sys_info.findState == 2) {
                    $(parent.pFind).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GREEN);
                } else {
                    $(parent.pFind).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GRAY);
                }
                //Alarm
                if (result.sys_info.alarmState == 3) {
                    $(parent.pAlarm).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].RED_BLINK);
                } else {
                    $(parent.pAlarm).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].GRAY);
                }
                //MGMT 
...
HomeController.prototype.selectTotalFlow = function () {
    ...
    try {
        ...
        var promise = URLManager.getInstance().ajaxSyncCall(link, { timeDelta:"1"});
        promise.fail(function (jqXHR, textStatus, err) {
            console.log(textStatus + " - " + err.message);
        });
        promise.done(function (result) {
