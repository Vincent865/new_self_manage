function NettopoController(viewHandle, elementId) {
    ...
    //set backGround
    this.imgPath = '/static/themes/images/img/';
    this.bgImage = this.imgPath+'bgpic.jpg';
...
NettopoController.prototype.initControl = function () {
    try {
        ...
        //构建网络特殊直线
        $(parent.pNetTopoImg).attr("src", Constants.PORT_IMAGETYPE[APPCONFIG.PRODUCT].TOPO);
        ...
        cloudNode.setImage(parent.imgPath + 'cloud_side.png', true);
...
NettopoController.prototype.buildNode = function (num, nodeObject, numList, scene) {
    try {
        ...
        if (nodeObject[4] == 1) {
            node.setImage(parent.imgPath + "warning_device_side.png", true);
        }
        else {
            node.setImage(parent.imgPath + "normal_device_side.png", true);
        }
        node.radius = 15;
        ...
        //自定义属性
        node.text = nodeObject[0];
        //node.deviceId = nodeObject[0];
        node.deviceMAC = nodeObject[1];
        node.deviceIP = nodeObject[2];
        node.topId = nodeObject[5];
        var protocal = "";
        var nodeObj = (nodeObject[3] + "").split(",");
        nodeObj.sort();
        ...
        node.click(function (event) {
            ...
            $("#btnUpdate").bind("click", function () {
                try {
                    ...
                    var mac = $("#txtDeviceMAC").val();
                    var topid = $("#txtDeviceName").attr("top-key");
                    ...
                    var link = APPCONFIG[APPCONFIG.PRODUCT].HTTP_URL + APPCONFIG[APPCONFIG.PRODUCT].SAVE_TOPO_DEVICES;
                    var data = { ip: ip, mac: mac, name: name, top_id: topid };
...
NettopoController.prototype.getTopoDevices = function (scence) {
    try {
        var parent = this;
        var link = APPCONFIG[APPCONFIG.PRODUCT].HTTP_URL + APPCONFIG[APPCONFIG.PRODUCT].GET_TOPO_DEVICES;
...
NettopoController.prototype.getDeviceNodes = function () {
    try {
        ...
        var link = APPCONFIG[APPCONFIG.PRODUCT].HTTP_URL + APPCONFIG[APPCONFIG.PRODUCT].GET_TOPO_PATH;
        var data = { mac: currentItem.deviceMAC, ip: currentItem.deviceIP };
        ...
        promise.done(function (result) {
            if (typeof (result) != "undefined" && typeof (result.iplist) != "undefined" && typeof (result.maclist) != "undefined") {
                ...
                for (var i = 0; i < arryNodes.length; i++) {
                    if (arryNodes[i].elementType == "link") {
                        ...
                        if (arryNodes[i].text != "" && arryNodes[i].deviceMAC != null) {
                            for (var j = 0; j < result.iplist.length; j++) {
                                if (arryNodes[i].deviceMAC == result.maclist[j] && arryNodes[i].deviceMAC != currentScene.currentElement.deviceMAC &&
                                    arryNodes[i].deviceIP == result.iplist[j] && arryNodes[i].deviceIP != currentScene.currentElement.deviceIP) {
                ...
                for (var n = 0; n < temLinks.length; n++) {
                    for (var m = 0; m < temNodes.length; m++) {
                        if (temLinks[n].nodeZ.deviceMAC == temNodes[m].deviceMAC && temLinks[n].nodeZ.deviceIP == temNodes[m].deviceIP) {
...
ContentFactory.assignToPackage("tdhx.base.nettopo.NettopoController", NettopoController);
