function CusProtocolController(controller, viewHandle, elementId, id, ip, mac, port, time, protocol) {
    ...
    this.pLBDESC = "#" + this.elementId + "_lblDESC";
...
CusProtocolController.prototype.init = function () {
    try {
        var parent = this;

        var tabTemplate = TemplateManager.getInstance().updateTemplate(TemplateManager.getInstance().getTemplate(Constants.TEMPLATES.AUDIT_PROTOCOL_CUSPROTO), {
        ...
        console.error("ERROR - CusProtocolController.init() - Unable to initialize: " + err.message);
    }
}

CusProtocolController.prototype.initControls = function () {
    try {
    }
    catch (err) {
        console.error("ERROR - CusProtocolController.initControls() - Unable to initialize control: " + err.message);
    }
}

CusProtocolController.prototype.load = function () {
    try {
        //details
        var parent = this;
        ...
        console.error("ERROR - CusProtocolController.load() - Unable to load: " + err.message);
    }
}

CusProtocolController.prototype.selectProtocol = function () {
    var html = "";
    var parent = this;
    try {
            ...
            if (typeof (result) != "undefined" && typeof (result.rows) != "undefined") {
                if (result.rows.length > 0) {
                    for (var i = 0; i < result.rows.length; i++) {
                        html += "<tr>";
                        html += "<td style='width:20%;'>" + result.rows[i]['date'] + "</td>";
                        html += "<td style='text-align:left;'>" + result.rows[i]['detail'] + "</td>";
                        html += "</tr>";
                    }
                }
                else {
        ...
        $(parent.pTdProtocolList + ">tbody").html(html);
        console.error("ERROR - CusProtocolController.selectProtocol() - Unable to get all events: " + err.message);
    }
}

ContentFactory.assignToPackage("tdhx.base.audit.CusProtocolController", CusProtocolController);
