function FOCASController(controller, viewHandle, elementId, id, ip, mac, port, time, protocol) {
...
FOCASController.prototype.init = function () {
    try {
        var parent = this;

        var tabTemplate = TemplateManager.getInstance().updateTemplate(TemplateManager.getInstance().getTemplate(Constants.TEMPLATES.AUDIT_PROTOCOL_FOCAS), {
        ...
        console.error("ERROR - FOCASController.init() - Unable to initialize: " + err.message);
    }
}

FOCASController.prototype.initControls = function () {
    try {
    }
    catch (err) {
        console.error("ERROR - FOCASController.initControls() - Unable to initialize control: " + err.message);
    }
}

FOCASController.prototype.load = function () {
    try {
        //details
        var parent = this;
        ...
        console.error("ERROR - FOCASController.load() - Unable to load: " + err.message);
    }
}

FOCASController.prototype.selectProtocol = function () {
    var html = "";
    var parent = this;  
    try {
            ...
            if (typeof (result) != "undefined" && typeof (result.rows) != "undefined") {
                if (result.rows.length > 0) {
                    for (var i = 0; i < result.rows.length; i++) {
                        html += "<tr>";
                        html += "<td style='width:20%;'>" + result.rows[i][7] + "</td>";
        ...
        html += "<tr><td colspan='5'>暂无数据</td></tr>";
        $(parent.pTdProtocolList + ">tbody").html(html);
        console.error("ERROR - FOCASController.selectProtocol() - Unable to get all events: " + err.message);
    }
}

ContentFactory.assignToPackage("tdhx.base.audit.FOCASController", FOCASController);