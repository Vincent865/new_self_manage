function PNRTDCPController(controller, viewHandle, elementId, id, ip, mac, port, time, protocol) {
...
PNRTDCPController.prototype.selectProtocol = function () {
    var html = "";
    var parent = this;
    try {
            ...
            if (typeof (result) != "undefined" && typeof (result.rows) != "undefined") {
                if (result.rows.length > 0) {
                    for (var i = 0; i < result.rows.length; i++) {
                        html += "<tr>";
                        html += "<td style='width:20%;'>" + result.rows[i][8] + "</td>";
...
ContentFactory.assignToPackage("tdhx.base.audit.PNRTDCPController", PNRTDCPController);