function SIPController(controller, viewHandle, elementId, id, ip, mac, port, time, protocol) {
...
SIPController.prototype.init = function () {
    try {
        var parent = this;

        var tabTemplate = TemplateManager.getInstance().updateTemplate(TemplateManager.getInstance().getTemplate(Constants.TEMPLATES.AUDIT_PROTOCOL_SIP), {
        ...
        console.error("ERROR - SIPController.init() - Unable to initialize: " + err.message);
    }
}

SIPController.prototype.initControls = function () {
    try {
    }
    catch (err) {
        console.error("ERROR - SIPController.initControls() - Unable to initialize control: " + err.message);
    }
}

SIPController.prototype.load = function () {
    try {
        //details
        var parent = this;
        ...
        console.error("ERROR - SIPController.load() - Unable to load: " + err.message);
    }
}

SIPController.prototype.selectProtocol = function () {
    var html = "";
    var parent = this;
    try {
            ...
            if (typeof (result) != "undefined" && typeof (result.rows) != "undefined") {
                if (result.rows.length > 0) {
                    for (var i = 0; i < result.rows.length; i++) {
                        html += "<tr>";
                        html += "<td style='width:20%;'>" + result.rows[i][14] + "</td>";
        ...
        html += "<tr><td colspan='2'>暂无数据</td></tr>";
        $(parent.pTdProtocolList + ">tbody").html(html);
        console.error("ERROR - SIPController.selectProtocol() - Unable to get all events: " + err.message);
    }
}

ContentFactory.assignToPackage("tdhx.base.audit.SIPController", SIPController);