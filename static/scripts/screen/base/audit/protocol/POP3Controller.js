function POP3Controller(controller, viewHandle, elementId, id, ip, mac, port, time, protocol) {
...
POP3Controller.prototype.selectProtocol = function () {
    var html = "";
    var parent = this;
    try {
            ...
            if (typeof (result) != "undefined" && typeof (result.rows) != "undefined") {
                if (result.rows.length > 0) {
                    for (var i = 0; i < result.rows.length; i++) {
                        ...
                        html += "<td style='text-align:left;'>" + result.rows[i][2].replace(/</g,'&lt;').replace(/>/g,'&gt;') + "</td>";
                        html += "</tr>";
                    }
                }
                else {
...
ContentFactory.assignToPackage("tdhx.base.audit.POP3Controller", POP3Controller);