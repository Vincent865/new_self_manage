function ORACLEController(controller, viewHandle, elementId, id, ip, mac, port, time, protocol) {
...
ORACLEController.prototype.init = function () {
    try {
        var parent = this;

        var tabTemplate = TemplateManager.getInstance().updateTemplate(TemplateManager.getInstance().getTemplate(Constants.TEMPLATES.AUDIT_PROTOCOL_ORACLE), {
        ...
        console.error("ERROR - ORACLEController.init() - Unable to initialize: " + err.message);
    }
}

ORACLEController.prototype.initControls = function () {
    try {
    }
    catch (err) {
        console.error("ERROR - ORACLEController.initControls() - Unable to initialize control: " + err.message);
    }
}

ORACLEController.prototype.load = function () {
    try {
        //details
        var parent = this;
        ...
        console.error("ERROR - ORACLEController.load() - Unable to load: " + err.message);
    }
}

ORACLEController.prototype.selectProtocol = function () {
    var html = "";
    var parent = this;
    try {
        ...
        html += "<tr><td colspan='4'>暂无数据</td></tr>";
        $(parent.pTdProtocolList + ">tbody").html(html);
        console.error("ERROR - ORACLEController.selectProtocol() - Unable to get all events: " + err.message);
    }
}

ContentFactory.assignToPackage("tdhx.base.audit.ORACLEController", ORACLEController);