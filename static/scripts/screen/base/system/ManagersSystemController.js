 ...
 */
function ManagersSystemController(controller,viewHandle, elementId) {
...
ManagersSystemController.prototype.init = function () {
    try {
        var parent = this;
        var tabTemplate = TemplateManager.getInstance().updateTemplate(TemplateManager.getInstance().getTemplate(Constants.TEMPLATES.SYSTEM_MANAGERS), {
...
ManagersSystemController.prototype.initControls = function () {
    try {
        var parent = this;
...
ManagersSystemController.prototype.load = function (info) {
    try {
    }
    catch (err) {
...
ManagersSystemController.prototype.commonExt = function (info,link,layerIndex,promise,spec) {
    try {
        //var sibNodes=$(this.pBtnTestStatus).siblings('input'),serverIp =sibNodes.eq(0).val(),serverPort=sibNodes.eq(1).val();
        var serverIp=$(this.serverIp).val(),serverPort=$(this.serverPort).val(),enabledStatus=Number($('#launch_monitor').prop('checked'));
        if((serverIp&&serverPort)&&((Validation.validateIP(serverIp)||Validation.validateIPV6(serverIp))&&Validation.validatePort(serverPort))){
            if(serverIp.slice(0,4).toLowerCase()==='fe80'){
                layer.alert('配置IPV6地址不能为本地链路地址',{icon:5});
                return false;
            }
            ...
            !spec?promise = URLManager.getInstance().ajaxCallByURL(link,'POST',{ip:serverIp.toLowerCase(),port:serverPort,status:enabledStatus}):promise = URLManager.getInstance().ajaxCallByURL(link,'POST');
            return promise;
        }else{
...
ManagersSystemController.prototype.testStatus = function () {
    try {
...
ManagersSystemController.prototype.getConnectStatus = function () {
    try {
        var parent = this,layerIndex;
        var link = APPCONFIG[APPCONFIG.PRODUCT].HTTP_URL+APPCONFIG[APPCONFIG.PRODUCT].GET_REGISTER_STATUS;
                ...
                if (typeof (result) != "undefined" && typeof (result.status) != "undefined") {
                    switch (result.status) {
                        case 1:
                            ...
                            result.res.status?$('#launch_monitor').prop({checked:true}):$('#launch_monitor').prop({checked:false});
...
ManagersSystemController.prototype.connectDevice = function () {
    try {
        var parent = this,layerIndex,promise;
        var link = APPCONFIG[APPCONFIG.PRODUCT].HTTP_URL+APPCONFIG[APPCONFIG.PRODUCT].CONNECT_DEVICE;
...
ManagersSystemController.prototype.disconnectDevice = function () {
    try {
        var parent = this,layerIndex,promise;
...
ContentFactory.assignToPackage("tdhx.base.system.ManagersSystemController", ManagersSystemController);
