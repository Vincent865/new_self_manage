...
MAP_DICT = {"srcIp": u"源IP", "srcPort": u"源端口", "srcMac": u"源MAC", "destIp": u"目的IP",
            ...
            "srcname": u"源设备名", "dstname": u"目的设备名", "signaturename": u"事件来源", "risklevel": u"危害等级",
            "dev_ip4": u"ipv4地址","dev_ip6": u"ipv6地址", "second_floor": u"是否包含二层设备"}
order_list = ["start_time", "end_time", "srcname", "srcIp", "srcMac", "srcPort", "srcaddr", "dstname", "destIp",
              "destMac", "destPort", "dstaddr", "protocol", "proto", "signaturename", "risklevel", "dev_ip4", "dev_ip6" , "second_floor"]
EVENT_SRC_DICT = {"1": "white_list", "2": "black_list", "3": "IPMAC", "4": "traffic_alert", "5": "mac_filter", "6": "dev_alert", "7": "abnormal_packet_alert"}
# EVENT_EN_CH_DICT = {"white_list": u"白名单", "black_list": u"黑名单", "IPMAC": u"IP/MAC", "traffic_alert": u"流量告警"}
EVENT_SRC_DICT_CH = {1: u"白名单", 2: u"黑名单", 3: u"IP/MAC", 4: u"流量告警", 5: u"MAC过滤", 6: u"资产告警", 7:U"不合规报文告警"}
RISK_LEVEL_CH = {0: u"低危", 1: u"中危", 2: u"高危"}
...
class ReportBuild:
    ...
    def base_info_table(self, story):
        ...
        logger.info("input_paragram+++++++++" + str(input_paragram))
        in_param = collections.OrderedDict()
        for elem in order_list:
            if elem in input_paragram:
                in_param[elem] = input_paragram[elem]
        if "signaturename" in in_param:
            in_param["signaturename"] = EVENT_SRC_DICT_CH[int(in_param["signaturename"])]
        if "risklevel" in in_param:
            in_param["risklevel"] = RISK_LEVEL_CH[int(in_param["risklevel"])]
        if "second_floor" in in_param:
            in_param["second_floor"] = EQUIPMENT_LEVEL[int(in_param["second_floor"])]
        logger.info("in_param=============" + str(in_param))
        for k, v in in_param.iteritems():
            if len(v) > 40:
                b = ''
    ...
    def src_dst_table(self, story):
        ...
        for flow_data in self.data_dict["proto_order"]:
            cell=flow_data[0]+"<-->"+flow_data[1]
            if len(cell) > 45:
                cell = flow_data[0] + "\n<-->\n" + flow_data[1]

            # data.append([flow_data[0]+"<-->"+flow_data[1], int(flow_data[2])])
            data.append([cell, int(flow_data[2])])
        t = Table(data, colWidths=[300, 60], spaceAfter=6)
    ...
    def proto_go(self):
        ...
        try:
            ...
            logger.info("====================enter proto report multiBuild.............")
    ...
    def event_go(self):
        ...
        add_title(story, '1 前言', h1)
        add_title(story, '1.1 报告阅读说明', h2)
        story.append(Paragraph('本报告是安全事件情况的统计，主要内容包含攻击源/目标TOP5，安全事件协议分布，安全事件来源分布，'
        ...
        try:
            ...
            logger.info("====================enter event report multiBuild.............")
            doc.multiBuild(story, onFirstPage=firstPages, onLaterPages=laterPages)
            os.system("rm -rf " + path + "/tmp/")
    ...
    def oper_figure(self, story):
        ...
        if json_dict["common"] == 0:
            json_dict.pop("common")
        json_string = json.dumps(json_dict)
        cmd = phantomjs_path + ' ' + js_path + ' -url ' + \
              path + '/js/charts/tpl/index_event_1.html -width 500 -height 390 -json ' + "'" + \
    ...
    def log_go(self):
        ...
        try:
            ...
            logger.info("====================enter log report multiBuild.............")
            doc.multiBuild(story, onFirstPage=firstPages, onLaterPages=laterPages)
            os.system("rm -rf " + path + "/tmp/")
    ...
    # 设备在线情况分布饼状图
    def assets_live(self, story):
        for i in self.data_dict["dev_live_info"].keys():
            if i == "0":
                self.data_dict["dev_live_info"]["UNLIVEDEV"] = self.data_dict["dev_live_info"].pop(i)
            elif i == "1":
                self.data_dict["dev_live_info"]["LIVEDEV"] = self.data_dict["dev_live_info"].pop(i)
        json_string = str(json.dumps(self.data_dict["dev_live_info"]).decode("unicode-escape")).replace("'","\"").strip()
        cmd = phantomjs_path + ' ' + js_path + ' -url ' + \
              path + '/js/charts/tpl/index_event_1.html -width 500 -height 390 -json ' + "'" + \
              json_string + "' -outfile " + path + "/tmp/assets_live.png"
        os.system(cmd)
        story.append(Image(path + '/tmp/assets_live.png', width=400, height=312))
        story.append(Paragraph("图 <seq template='2.1'/> 设备在线数量分布图", figure_name))

    # 设备类型分布饼状图
    def assets_type(self, story):
        for i in self.data_dict["dev_type_info"].keys():
            if i == "0":
                self.data_dict["dev_type_info"]["UNKNOWNTYPE"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "1":
                self.data_dict["dev_type_info"]["PC"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "2":
                self.data_dict["dev_type_info"]["ENGINEERPC"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "3":
                self.data_dict["dev_type_info"]["OPERATERPC"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "4":
                self.data_dict["dev_type_info"]["PLC"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "5":
                self.data_dict["dev_type_info"]["RTU"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "6":
                self.data_dict["dev_type_info"]["HMI"] = self.data_dict["dev_type_info"].pop(i)
            elif i == "7":
                self.data_dict["dev_type_info"]["NETPC"] = self.data_dict["dev_type_info"].pop(i)

        logger.info(self.data_dict)
        json_string = str(json.dumps(self.data_dict["dev_type_info"]).decode("unicode-escape")).replace("'", "\"").strip()
        cmd = phantomjs_path + ' ' + js_path + ' -url ' + \
              path + '/js/charts/tpl/index_event_2.html -width 500 -height 390 -json ' + "'" + \
              json_string + "' -outfile " + path + "/tmp/assets_type.png"
        os.system(cmd)
        story.append(Image(path + '/tmp/assets_type.png', width=400, height=312))
        story.append(Paragraph("图 <seq template='2.2'/> 设备类型分布图", figure_name))

    # 设备身份分布饼状图
    def assets_status(self, story):
        for i in self.data_dict["dev_status_info"].keys():
            if i == "1":
                self.data_dict["dev_status_info"]["UNKNOWN"] = self.data_dict["dev_status_info"].pop(i)
            elif i == "0":
                self.data_dict["dev_status_info"]["KNOWN"] = self.data_dict["dev_status_info"].pop(i)
        json_string = str(json.dumps(self.data_dict["dev_status_info"]).decode("unicode-escape")).replace("'","\"").strip()
        cmd = phantomjs_path + ' ' + js_path + ' -url ' + \
              path + '/js/charts/tpl/index_event_1.html -width 500 -height 390 -json ' + "'" + \
              json_string + "' -outfile " + path + "/tmp/assets_status.png"
        os.system(cmd)
        story.append(Image(path + '/tmp/assets_status.png', width=400, height=312))
        story.append(Paragraph("图 <seq template='2.3'/> 设备身份状态分布图", figure_name))
    ...
    def assets_go(self):
        ...
        add_title(story, '1 前言', h1)
        add_title(story, '1.1 报告阅读说明', h2)
        story.append(Paragraph('本报告是资产报告情况的统计，设备在线统计分布，设备类型统计分布，设备身份状态统计分布。', body))
        ...
        try:
            ...
            logger.info("====================enter assets report multiBuild.............")
            doc.multiBuild(story, onFirstPage=firstPages, onLaterPages=laterPages)
            os.system("rm -rf " + path + "/tmp/")
            os.system("mv " + path + "/pdf_tmp/assets_report.pdf " + "/data/report/assets/")
        except:
            logger.error("MyDocTemplate build error.")
            logger.error(traceback.format_exc())
    ...
    def run(self):
        try:
            ...
            if self.report_type == "audit":
                ...
                os.system("cp " + path + "/Assets/cover_audit.png " + path + "/Assets/cover.png")
                self.proto_go()  # 生成审计报告
            elif self.report_type == "event":
                logger.info('enter generate event report=================')
                os.system("cp " + path + "/Assets/cover_event.png " + path + "/Assets/cover.png")
                self.event_go()  # 生成事件报告
            elif self.report_type == "log":
                logger.info('enter generate log report=================')
                os.system("cp " + path + "/Assets/cover_log.png " + path + "/Assets/cover.png")
                self.log_go()  # 生成日志报告
            elif self.report_type == "assets":
                logger.info('enter generate assets report=================')
                os.system("cp " + path + "/Assets/cover_assets.png " + path + "/Assets/cover.png")
                self.assets_go()  # 生成资产报告
            else:
